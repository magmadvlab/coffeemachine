      ...
      const res = await fetch(`/api/riparazioni/${id}`, {
        ...
        body: JSON.stringify({
          ...
          scheda: {
            ...
            accessori,
            note_stato: formData.get("note_stato"),
          ...
          diagnosi_tecnico: formData.get("diagnosi_tecnico"),
          importo_preventivo: formData.get("importo_preventivo"),
          importo_finale: formData.get("importo_finale"),
        }),
      });
  ...
  return (
    <details className="rounded-2xl border border-coffee-100 bg-white p-4 shadow-sm shadow-coffee-900/5 sm:p-5">
      ...
      <form action={save} className="mt-4 space-y-5">
        ...
        <section>
          <h3 className="mb-3 text-sm font-bold text-coffee-900">Scheda</h3>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
            ...
            <div>
              <label className={labelCls}>Preventivo richiesto?</label>
              <select
            ...
            <div>
              <label className={labelCls}>Preventivo</label>
              <input name="importo_preventivo" type="number" min="0" step="0.01" defaultValue={scheda.importo_preventivo?.toString() ?? ""} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Importo finale</label>
              <input name="importo_finale" type="number" min="0" step="0.01" defaultValue={scheda.importo_finale?.toString() ?? ""} className={inputCls} />
            </div>
            <div className="sm:col-span-2">
            ...
            <div className="sm:col-span-2">
              <label className={labelCls}>Note stato/accessori</label>
              <textarea name="note_stato" defaultValue={scheda.note_stato ?? ""} rows={3} className={inputCls} />
            </div>
            <div className="sm:col-span-2">
              <label className={labelCls}>Difetto segnalato dal cliente</label>
              <textarea name="difetto_cliente" defaultValue={scheda.difetto_cliente ?? ""} rows={4} className={inputCls} />
            </div>
            <div className="sm:col-span-2">
              <label className={labelCls}>Note tecniche / lavoro svolto</label>
              <textarea name="diagnosi_tecnico" defaultValue={scheda.diagnosi_tecnico ?? ""} rows={4} className={inputCls} />
            </div>
          </div>
        </section>
