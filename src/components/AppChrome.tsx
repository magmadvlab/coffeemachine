...
export function AppChrome({ children, isAdmin = false }: { children: React.ReactNode; isAdmin?: boolean }) {
  const pathname = usePathname();
  if (shouldHideChrome(pathname)) return <>{children}</>;
  const visiblePrimaryLinks = isAdmin ? primaryLinks : operatorPrimaryLinks;
  const visibleUtilityLinks = isAdmin ? utilityLinks : operatorUtilityLinks;
  const mobileLinks = isAdmin ? [...primaryLinks, utilityLinks[0]] : [...operatorPrimaryLinks, ...operatorUtilityLinks];
      ...
      <aside className="fixed left-0 top-0 z-40 hidden h-screen w-64 flex-col bg-coffee-900 px-4 py-5 text-white shadow-xl lg:flex">
        <Link href="/" className="mb-5 flex items-start gap-3 rounded-2xl bg-white/8 p-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-arancio text-white">
            <img src="/symbol.png" alt="" className="h-7 w-7 object-contain" />
          </span>
          <span className="leading-tight">
            <span className="block font-display text-lg font-bold">Coffee Express</span>
            <span className="block text-xs font-semibold text-white/55">{isAdmin ? "Gestionale" : "Officina"}</span>
          </span>
        </Link>
